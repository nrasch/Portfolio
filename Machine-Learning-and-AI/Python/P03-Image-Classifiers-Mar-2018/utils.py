...
def initilizeParameters(dimensionDict, multiplier):
    ...
    for index, size in enumerate(dimensionDict['hiddenLayerSizes'], start=1):
        ...
        #params[wName] = np.random.randn(size, lastDimSize) * multiplier    
    ...
    #params[wName] = np.random.randn(dimensionDict["numberOutputs"], lastDimSize) * multiplier
    # Initialize utilizing "He Initialization"
    np.random.seed(10)  # Yes, this has to be done every time...  :(
...
def forwardPropagation(data, params):
    ...
    for i in range(1, numLayers + 1):
        ...
        # Perform sigmoid or tanh activation
        if (i == numLayers):
...
def backwardPropagation(labels, cache, params, lamb):
    ...
    for i in reversed(range(1, layers)):
        ...
        dz = np.dot(params[wBefore].T, dzBefore) * (1 - np.power(cache[a], 2))
        grads[dw], grads[db] = linearBackProp(dz, cache[aPrev], params[w], params[b], lamb)
...
def model(data, labels, dims, numIterations, learningRate, lamb, initializeMultiplier, printCost = False, showGraph = False):
    
    # Init vars
    #dims = defineDimensions(data, labels, layerSizes)
    ...
    for i in range(0, numIterations + 1):
        ...
        grads = backwardPropagation(labels, cache, params, lamb)
